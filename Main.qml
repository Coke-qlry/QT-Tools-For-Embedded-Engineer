...
Window {
    ...
    function sortDevices() {
        ...
        deviceModel.clear()
        for (var j = 0; j < arr.length; j++) deviceModel.append(arr[j])
        root.showToast(sortByName ? "已按名称排序" : "已按信号强度排序")
    }
    function connectToDevice(index) {
        var it = deviceModel.get(index)
        if (!it) return
        if (it.connected) { bleManager.disconnectFromDevice(); return }
        // 需要设备的地址；连接成功后由 connectedChanged 更新该行状态
        root.targetAddress = it.address
    ...
    function saveEdit() {
        ...
        editDialog.close()
        showToast(editDialog.isNew ? "已新增指令" : "已保存修改")
    }
        ...
        Rectangle {
            ...
            ToolButton {
                ...
                contentItem: Column {
                    spacing: 2
                    anchors.centerIn: parent
                    Text { text: "⇅"; font.pixelSize: 15; color: root.cText; horizontalAlignment: Text.AlignHCenter; anchors.horizontalCenter: parent.horizontalCenter }
                    Text { text: "排序(Sort)"; font.pixelSize: 9; color: root.cText; horizontalAlignment: Text.AlignHCenter; anchors.horizontalCenter: parent.horizontalCenter }
                }
            }
            // 清除
            ToolButton {
                ...
                contentItem: Column {
                    ...
                    anchors.centerIn: parent
                    Text { text: "✕"; font.pixelSize: 15; color: root.cText; horizontalAlignment: Text.AlignHCenter; anchors.horizontalCenter: parent.horizontalCenter }
                    Text { text: "清除(Clear)"; font.pixelSize: 9; color: root.cText; horizontalAlignment: Text.AlignHCenter; anchors.horizontalCenter: parent.horizontalCenter }
                }
            }
        }
                ...
                delegate: Rectangle {
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                anchors.fill: parent
                                onClicked: root.connectToDevice(index)   // 连接/断开设备
                            }
                        }
                    }
                    ...
                    MouseArea {
                        id: mouseArea
                        anchors.fill: parent
                        onClicked: root.connectToDevice(index)
                    }
                    ...
                    model: [
                        ...
                        delegate: Item {
                            ...
                            MouseArea {
                                anchors.fill: parent
                                onClicked: {
                                    if (index === 2) commandPage.visible = true   // 指令 → 进入子界面
                                    else root.currentTab = index > 2 ? index - 1 : index
                                }
                            }
                        Column {
                            anchors.centerIn: parent
    ...
    Item {
        ...
        Rectangle {
            ...
            ToolButton {
                ...
                implicitWidth: 44
                implicitHeight: 44
                onClicked: commandPage.visible = false
                contentItem: Text {
                    text: "‹"
                    font.pixelSize: 30
                    color: root.cText
    ...
    Connections {
        ...
        function onConnectedChanged(v) {
            // 依据连接状态更新设备列表对应行
            for (var i = 0; i < deviceModel.count; i++) {
                if (deviceModel.get(i).address === root.targetAddress)
